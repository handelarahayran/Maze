...
# Control the settings of the BFS algorithm:
...
class Node:
    def __init__(self, loc: list[2], parent: int):
        self.location = loc     # The location in the maze
        self.parent = parent    # The parent node index in the tree list
...
# Initialize the Tree and find the first Node - start
Tree = []
        ...
        for i in range(len(movement)):
            ...
            if check(Tree[pointer].location + movement[i], maze_size):
                ...
                if search(Tree[pointer].location + movement[i], pointer):
                    ...
                    raise Found
        # Check around the next "oldest" Node (BFS) in the next loop
        pointer += 1
